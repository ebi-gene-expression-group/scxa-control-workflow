...
nextflowCommand="nextflow run -N $SCXA_REPORT_EMAIL -r $scxaBranch -resume workflows/${workflow}/main.nf $expNamePart $skipQuantificationPart $skipAggregationPart $tertiaryWorkflowPart $overwritePart --enaSshUser fg_atlas_sc --sdrfDir $SCXA_SDRF_DIR -work-dir $workingDir"
